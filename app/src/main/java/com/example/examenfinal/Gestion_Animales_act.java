...
import com.example.examenfinal.database.AdminSQLiteOpenHelper;
...
public class Gestion_Animales_act extends AppCompatActivity {
    ...
    public void añadirFicha(View view){
        ...
        if(!tipoAnimal.isEmpty() && !edadAnimal.isEmpty() && !enfermedadAnimal.isEmpty()){
    ...
    public void mostrarFicha(View view){
        ...
        if(!fichaAnimal.isEmpty()){
            ...
            Cursor file = db.rawQuery("SELECT nombre, tipo, edad, enfermedad FROM animales WHERE ficha="+fichaAnimal, null);

            if(file.moveToFirst())
            {
                nombre.setText(file.getString(0));
                enfermedad.setText(file.getString(1));
